...
namespace NBitcoin
{
    public partial class Network
    {
        ...
        private static Network InitImpleumMain()
        {
            ...
            Network network = new Network
            {
                Name = nameof(ImpleumMain),
                RootFolderName = ImpleumRootFolderName,
                DefaultConfigFilename = ImpleumDefaultConfigFilename,
                Magic = magic,
                DefaultPort = 16171,
                RPCPort = 16172,
                MinTxFee = 10000,
                FallbackFee = 60000,
                MinRelayTxFee = 10000,
                MaxTimeOffsetSeconds = ImpleumMaxTimeOffsetSeconds,
                MaxTipAge = ImpleumDefaultMaxTipAgeInSeconds
            };
            ...
            network.Consensus.LastPOWBlock = 100000;
            network.Consensus.IsProofOfStake = true;
            network.Consensus.ConsensusFactory = new PosConsensusFactory() { Consensus = network.Consensus };
            ...
            network.Consensus.CoinType = 769;
            ...
            Assert(network.Consensus.HashGenesisBlock == uint256.Parse("0x02a8be139ec629b13df22e7abc7f9ad5239df39efaf2f5bf3ab5e4d102425dbe"));
            Assert(genesis.Header.HashMerkleRoot == uint256.Parse("0xbd3233dd8d4e7ce3ee8097f4002b4f9303000a5109e02a402d41d2faf74eb244"));


            network.Base58Prefixes[(int)Base58Type.PUBKEY_ADDRESS] = new byte[] { (102) };
            network.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS] = new byte[] { (125) };
            network.Base58Prefixes[(int)Base58Type.SECRET_KEY] = new byte[] { (63 + 128) };
            network.Base58Prefixes[(int)Base58Type.ENCRYPTED_SECRET_KEY_NO_EC] = new byte[] { 0x01, 0x42 };
            network.Base58Prefixes[(int)Base58Type.ENCRYPTED_SECRET_KEY_EC] = new byte[] { 0x01, 0x43 };
            ...
            string[] seeds = new[] { "109.108.77.134", "62.80.181.141" };
        ...
        private static Network InitImpleumRegTest()
        {
            ...
            Block genesis = CreateImpleumGenesisBlock(network.Consensus.ConsensusFactory, 1523364655, 2380297, 0x1e0fffff, 1, Money.Zero);
            genesis.Header.Time = 1527013886;
            genesis.Header.Nonce = 2112374;
            genesis.Header.Bits = network.Consensus.PowLimit;
            network.genesis = genesis;
            network.Consensus.HashGenesisBlock = genesis.GetHash();
            //Assert(network.Consensus.HashGenesisBlock == uint256.Parse("0x93925104d664314f581bc7ecb7b4bad07bcfabd1cfce4256dbd2faddcf53bd1f"));
